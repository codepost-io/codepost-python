...
class TestGetAvailableCourses(object):
    ...
    def test_get_available_courses_on_success_without_filter(self):
        ...
        courses = helpers.get_available_courses(api_key=TEST_API_KEY)
    ...
    def test_get_available_courses_on_success_with_filter_results(self):
        ...
        courses = helpers.get_available_courses(api_key=TEST_API_KEY, course_name='COS126')
        _assert.assertEqual([course1], courses)

        # test filtering by period
        courses = helpers.get_available_courses(api_key=TEST_API_KEY, course_period='S2020')
        _assert.assertEqual([course2], courses)

        # test filtering by both name and period
        courses = helpers.get_available_courses(api_key=TEST_API_KEY, course_name='COS226', course_period='S2020')
        _assert.assertEqual([course2], courses)
    ...
    def test_get_available_courses_on_success_with_no_filter_results(self):
        ...
        # test filtering by name
        courses = helpers.get_available_courses(api_key=TEST_API_KEY, course_name='COS120')
        _assert.assertEqual([], courses)

        # test filtering by period
        courses = helpers.get_available_courses(api_key=TEST_API_KEY, course_period='S2000')
        _assert.assertEqual([], courses)

        # test filtering by both name and period
        courses = helpers.get_available_courses(api_key=TEST_API_KEY, course_name='COS326', course_period='S2020')
        _assert.assertEqual([], courses)
...
def test_get_course_roster_by_id(mock_get):
    ...
    response = helpers.get_course_roster_by_id(course_id=roster['id'], api_key=TEST_API_KEY)
...
def test_get_course_roster_by_name(mock_get_available_courses, mock_get_course_roster_by_id):
    ...
    response = helpers.get_course_roster_by_name('COS126', 'S2019', api_key=TEST_API_KEY)
...
def test_get_assignment_by_id(mock_get):
    ...
    response = helpers.get_course_roster_by_id(course_id=assignment['id'], api_key=TEST_API_KEY,)
...
def test_get_assignment_submissions(mock_get):
    ...
    response = helpers.get_assignment_submissions(assignment_id=1, api_key=TEST_API_KEY)
...
def test_get_file(mock_get):
    ...
    response = helpers.get_file(file_id=file['id'], api_key=TEST_API_KEY)
...
def test_set_submission_grader(mock_patch):
    ...
    response = helpers.set_submission_grader(submission_id=submission1['id'], grader=None, api_key=TEST_API_KEY)
...
def test_unclaim_submission(mock_patch):
    ...
    response = helpers.unclaim_submission(submission_id=submission1['id'], api_key=TEST_API_KEY)
    _assert.assertEqual(True, response)
...
def test_remove_comments(mock_get, mock_delete):
    ...
    response = helpers.remove_comments(file_id=file1['id'], api_key=TEST_API_KEY)
    _assert.assertEqual(True, response)
...
def test_delete_submission(mock_delete):
    mock_delete.return_value.status_code = 204
    response = helpers.delete_submission(submission_id=1, api_key=TEST_API_KEY)
...
def test_delete_file(mock_delete):
    mock_delete.return_value.status_code = 204
    response = helpers.delete_file(file_id=1, api_key=TEST_API_KEY)
    _assert.assertIsNotNone(response)
...
def test_post_file(mock_post):
    ...
    response = helpers.post_file(1, 'test.txt', 'hello', 'txt', api_key=TEST_API_KEY)
    _assert.assertIsNotNone(response)
...
def test_post_submission(mock_post):
    ...
    mock_post.return_value.status_code = 201
    response = helpers.post_submission(1, ['test.txt'], [file], api_key=TEST_API_KEY)
    _assert.assertIsNotNone(response)
...
def test_post_comment(mock_post):
    ...
    mock_post.return_value.status_code = 201
    response = helpers.post_comment(file, 'test', 1, 0, 1, 0, 0, api_key=TEST_API_KEY)
    _assert.assertIsNotNone(response)
...
def test_set_submission_students(mock_patch):
    mock_patch.return_value.status_code = 200
    response = helpers.set_submission_students(1, ['student1@codepost.io'], api_key=TEST_API_KEY)
    _assert.assertIsNotNone(response)
...
def test_remove_students_from_submission(mock_patch, mock_delete):
    ...
    response = helpers.remove_students_from_submission(submission1, ['student1@codepost.io'], api_key=TEST_API_KEY)
    _assert.assertIsNotNone(response)

    # remove the last remaining student from submission => should trigger a delete
    response = helpers.remove_students_from_submission(submission2, ['student2@codepost.io'], api_key=TEST_API_KEY)
...
def test_get_course_grades(mock_roster, mock_assignment, mock_submissions):
    ...
    response = helpers.get_course_grades('COS126', 'S2019', api_key=TEST_API_KEY)
